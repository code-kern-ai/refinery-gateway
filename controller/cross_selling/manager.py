...
def update_cross_selling(cross_selling_id: str, name: str = None) -> CrossSelling:
    entity = cross_selling_bo.update(
        cross_selling_id, name=name, with_commit=True
    )
    if not entity:
        raise EntityNotFoundException(f"Cross selling {cross_selling_id} not found")
    return entity
...
def get_cross_selling_dict(cross_selling_id: str) -> dict:
    entity = get_cross_selling(cross_selling_id)
    return sql_alchemy_to_dict(entity, column_whitelist=CROSS_SELLING_WHITELIST)
